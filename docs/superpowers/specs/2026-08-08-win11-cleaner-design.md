...
清理完成后弹出结果对话框：成功/失败项数、失败原因、**总计释放空间**（成功删除项的大小总和，按项累加）。
...
├── core/
│   ├── deleter.py       # 删除引擎：删除前校验 + 永久删除 / 移入回收站（send2trash）+ 释放空间统计
...
### 删除前校验（core/deleter.py 内强制）

每个待删项在删除前逐项校验，任一条件不满足则跳过并标记失败，**宁可少删不可误删**：

1. **系统关键目录黑名单**：路径位于或指向 `C:\Windows`、`C:\Windows\System32`、`C:\Windows\SysWOW64`、`C:\Program Files`、`C:\Program Files (x86)` 关键子目录（白名单允许的除外）时拒绝删除。对 `Windows.old` 等合法目标，用精确路径匹配放行，不做通配
2. **正在运行的程序目录**：路径位于当前运行中进程的 exe 所在目录内时拒绝删除
3. **白名单校验**：路径不在 Cleaner 声明的允许范围内（Cleaner 必须在 scan 时声明每个 CleanItem 的来源类别与允许路径前缀，deleter 据此校验）
4. **不删除操作系统与已安装软件的目录本体**：仅允许删除临时缓存、日志等类型的内容

校验逻辑对四类清理项一视同仁，测试用假路径覆盖全部校验分支。
...
- 目录不存在：静默跳过
- 删除引擎：`shutil.rmtree`（永久）与 `send2trash`（回收站），失败返回原因展示在结果对话框；每个成功删除的项记录其扫描大小，累加为总计释放空间
...
6. 删除前校验生效：构造指向 Windows/System32/运行中程序目录的假路径，一律被拒绝删除，正常软件关键数据不被删除
7. 清理完成后结果对话框显示总计释放空间，数值等于成功删除项大小之和
